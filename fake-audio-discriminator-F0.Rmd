...
fd_valid_centered$coefs <- fd_valid$coefs -  rowMeans(fd_train$coefs)
```
...
# === 1. Separar las curvas por clase ===
idx_0 <- which(y_train == 0)
idx_1 <- which(y_train == 1)

fd_0 <- fd(coef = fd_train_centered$coefs[, idx_0],
           basisobj = fd_train_centered$basis)

fd_1 <- fd(coef = fd_train_centered$coefs[, idx_1],
           basisobj = fd_train_centered$basis)

# === 2. FPCA por clase ===
nharm <- 5 # cantidad de funciones principales a graficar

pca_0 <- pca.fd(fd_0, nharm = nharm)
pca_1 <- pca.fd(fd_1, nharm = nharm)

harm_0 <- pca_0$harmonics
harm_1 <- pca_1$harmonics

# === 3. Grilla común ===
t_grid <- seq(fd_train_centered$basis$rangeval[1],
              fd_train_centered$basis$rangeval[2],
              length.out = 400)

# === 4. Evaluar funciones principales por clase ===
eval_0 <- eval.fd(t_grid, harm_0)
eval_1 <- eval.fd(t_grid, harm_1)

# --- clase 0 ---
df0 <- data.frame(t = t_grid, eval_0)
colnames(df0) <- c("t", paste0("PC", 1:nharm))
...
# --- clase 1 ---
df1 <- data.frame(t = t_grid, eval_1)
colnames(df1) <- c("t", paste0("PC", 1:nharm))
...
# === 5. Graficar FPC para clase 0 ===
plot_clase0 <- ggplot(df0_long, aes(x = t, y = Valor, color = Componente)) +
  geom_line(size = 1.2) +
  theme_minimal() +
  labs(title = "Funciones Principales - Clase 0",
       x = "t", y = "Valor", color = "Componente") +
  scale_color_brewer(palette = "Dark2")

# === 6. Graficar FPC para clase 1 ===
plot_clase1 <- ggplot(df1_long, aes(x = t, y = Valor, color = Componente)) +
  geom_line(size = 1.2) +
  theme_minimal() +
  labs(title = "Funciones Principales - Clase 1",
       x = "t", y = "Valor", color = "Componente") +
  scale_color_brewer(palette = "Dark2")
