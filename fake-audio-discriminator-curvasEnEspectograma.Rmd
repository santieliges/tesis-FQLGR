...
La mayor crítica que podría hacer es que esto es masomenos como realizar la transformada de fourier para tomar el periodograma y luego vuelvo a hacer una especie de transformación de fourier cuando transformo en un functional data object en la base de fourier, por lo que la interpretación del espacio puede ser poco intuitiva
...
# --- Definir rutas ---
carpeta_training_fake <- 'D:/Tesis/archive/archive/for-norm/for-norm/training/fake'
carpeta_training_real <- 'D:/Tesis/archive/archive/for-norm/for-norm/training/real'

# --- Definir rutas ---
carpeta_testing_fake <- 'D:/Tesis/archive/archive/for-norm/for-norm/testing/fake'
carpeta_testing_real <- 'D:/Tesis/archive/archive/for-norm/for-norm/testing/real'
...
ls()  # para verificar que df_F0_training_completo y df_F0_testing_completo están en el entorno
...
# Ver distribución de la variable 'clase'
...
res_cv_pca <- grid_search_lambda_gradient_descent_penalized_PCA(fd_centered = fd_train_centered, y = Y_train, beta_init = beta_init, gamma_init = gamma_init, alpha_init = alpha_init,
                               basis = fd_train_centered$basis, LdPenalization = harmacclLfd, step_gradient = 0.01, iterations = 500,
  lambda_lin_values = c(0,10^seq(-7, 2, length.out = 10)),
  lambda_quad_values = c(0,10^seq(-7, 2, length.out = 10)), 
                               folds = 2, seq_var_threshold = c(0.7,0.8,0.85,0.99), seed = 123, batch_size = 31)

res_cv_noPCA <- grid_search_lambda_gradient_descent_penalized_noPCA(
  fd_centered = fd_train_centered, y = Y_train, 
  beta_init = beta_init, gamma_init = gamma_init, alpha_init = alpha_init,
                               basis = fd_train_centered$basis, LdPenalization = harmacclLfd,
                               step_gradient = 0.01, iterations = 500, 
   lambda_lin_values = c(0,10^seq(-7, 2, length.out = 10)),
  lambda_quad_values = c(0,10^seq(-7, 2, length.out = 10)), 
                               folds = 2, seed = 123 , batch_size = 31)

res_cv_plsr <- cv_grid_search_lambda_nt_kfold( fd_data_centered = fd_train_centered, y = Y_train,
                               basis = fd_train_centered$basis, LdPenalization = harmacclLfd, step_gradient = 0.01, iterations = 500,
  lambda_lin_values = c(0,10^seq(-7, 2, length.out = 10)),
  lambda_quad_values = c(0,10^seq(-7, 2, length.out = 10)), 
                               k_folds = 2, seed = 123 , batch_size = 31)
```
...
res_pca_noquad <- pca_coef_Upsilon1(fd_centered = fd_train_centered,y = Y_train, beta = beta_init, gamma = gamma_init, alpha = alpha_init, basis = estimationBasis, step_gradient = 0.002, iterations = 8000, var_threshold = 0.8, LdPenalization = 2, lambda_lin = 0, lambda_quad = 1e-3, modelo_quad = FALSE)

res_pca <- pca_coef_Upsilon1(fd_centered = fd_train_centered, y = Y_train, beta = res_pca_noquad$beta, gamma = gamma_init, alpha = alpha_init, basis = estimationBasis, step_gradient = 0.001, iterations = 20000, var_threshold = 0.9, LdPenalization = 2, lambda_lin = 0, lambda_quad = 0, tol = 1e-9, scale =  FALSE)
...
gamma_init <- gamma_init <- matrix(rnorm(nbasis^2, sd = 0.1), nrow = nbasis)
alpha_init <- 0
res_nopca <- gradient_descent_noPCA(fd_centered = fd_train_centered, y = Y_train, 
...
## Modelo sin PCA
```{r}
estimationBasis = fd_train_centered$basis
psi <- inprod(estimationBasis,estimationBasis)
evalbasis <- eval.basis(t,estimationBasis)


# Curva de convergencia
plot(res_noPCA$cost_history, type = "l", lwd = 2, col = "blue",
     xlab = "Iteraciones", ylab = "Negative Log-Likelihood",
     main = "Convergencia del descenso por gradiente")

# Comparar coeficientes recuperados vs verdaderos
plot(fd(coef = res_noPCA$beta,basisobj =  estimationBasis),  col = "blue", pch = 1)
legend("topright", legend = c("beta(t) Estimado"), col = c("blue"), lty = 1, lwd = 2)

evalbasis <- eval.basis(t,estimationBasis)
#evalbasis <- eval.fd(t,basis)
# Visualizar con persp (superficie 3D base)
gamma_est_NOpca <- evalbasis %*% res_noPCA$gamma %*% t(evalbasis )


persp(t, t, gamma_est_NOpca,
      main = "Superficie Estimada (sin PCA)", theta = 40, phi = 30, expand = 0.8 ,
      xlab = "s", ylab = "t", zlab = "gamma(s,t)", col = "lightgreen", ticktype = "detailed")

image(t, t, gamma_est_NOpca, main = "Gamma Estimada (sin PCA)", col = heat.colors(20))


# Medir accuracy de predicción
y_pred_final <- predict_log_func(fd_centered = fd_valid_centered, psi, res_noPCA$beta, res_noPCA$gamma, res_noPCA$alpha)
y_clasif <- ifelse(y_pred_final > 0.5, 1, 0)
accuracy <- mean(y_valid == y_clasif)
...
```{r}
# Curva de convergencia
plot(res_pca$cost_history, type = "l", lwd = 2, col = "blue",
     xlab = "Iteraciones", ylab = "Negative Log-Likelihood",
     main = "Convergencia del descenso por gradiente")


plot(fd(coef = res_pca$beta,basisobj =  estimationBasis),  col = "blue", pch = 1)
#legend("topright", legend = c("beta(t) Estimado"), col = c("blue"), lty = 1, lwd = 2)
lines(fd(coef = res_pca_noquad$beta,basisobj =  estimationBasis),  col = "red", pch = 1)
legend("topright", legend = c("PCA Quad", "PCA no-Quad"), col = c("blue", "red"), lty = 1, lwd = 2)

evalbasis <- eval.basis(t,estimationBasis)
#evalbasis <- eval.fd(t,basis)
# Visualizar con persp (superficie 3D base)
gamma_est_pca <- evalbasis %*% res_pca$gamma %*% t(evalbasis )
gamma_est_pca_noquad <- evalbasis %*% res_pca_noquad$gamma %*% t(evalbasis )



par(mfrow = c(1, 2))

persp(t, t, gamma_est_pca,
      main = "Superficie Estimada (PCA)", theta = 40, phi = 30, expand = 0.8 ,
      xlab = "s", ylab = "t", zlab = "gamma(s,t)", col = "lightgreen", ticktype = "detailed")
par(mfrow = c(1, 2))
image(t, t, gamma_est_pca_noquad, main = "PCA NO quad", col = heat.colors(20))
image(t, t, gamma_est_pca, main = "Gamma Estimada (PCA)", col = heat.colors(20))

# Medir accuracy de predicción
y_pred_final <- predict_log_func(fd_centered = fd_valid_centered, psi, res_pca$beta, res_pca$gamma, res_pca$alpha)
y_clasif <- ifelse(y_pred_final > 0.5, 1, 0)
accuracy <- mean(y_valid == y_clasif)
cat("Accuracy final:", round(accuracy, 3), "\n")
...
## Modelo con PLSR
Parece ser que el término cuadratico no aporta mucho en la regresión!
```{r}
# Curva de convergencia
plot(res_plsr_quad$cost_history, type = "l", lwd = 2, col = "blue",
     xlab = "Iteraciones", ylab = "Negative Log-Likelihood",
     main = "Convergencia del descenso por gradiente")
plot(res_plsr_noquad$cost_history, type = "l", lwd = 2, col = "red",
     xlab = "Iteraciones", ylab = "Negative Log-Likelihood",
     main = "Convergencia del descenso por gradiente")

# Comparar coeficientes recuperados vs verdaderos
plot(fd(coef = res_plsr_quad$beta,basisobj =  estimationBasis), col = "red", lwd = 2)
lines(fd(coef = res_plsr_noquad$beta,basisobj =  estimationBasis),  col = "blue", pch = 1)
legend("topright", legend = c("PLSR Quad", "PLSR no-Quad"), col = c("blue", "red"), lty = 1, lwd = 2)

evalbasis <- eval.basis(t,estimationBasis)
#evalbasis <- eval.fd(t,basis)
# Visualizar con persp (superficie 3D base)
gamma_est_plsr_quad <- evalbasis %*% res_plsr_quad$gamma %*% t(evalbasis )
gamma_est_plsr_noquad <- evalbasis %*% res_plsr_noquad$gamma %*% t(evalbasis )

par(mfrow = c(1, 2))


persp(t, t, gamma_est_plsr_quad,
      main = "PLSR Quad", theta = 40, phi = 30, expand = 0.8 ,
      xlab = "s", ylab = "t", zlab = "gamma(s,t)", col = "lightgreen",
      ticktype = "detailed")

par(mfrow = c(1, 2))  # 1 fila, 2 columnas

image(t, t, gamma_est_plsr_noquad, main = "PLSR NO quad", col = heat.colors(20))
image(t, t, gamma_est_plsr_quad, main = "PLSR Quad", col = heat.colors(20))


# Medir accuracy de predicción
y_pred_final <- predict_log_func(fd_centered = fd_valid_centered, psi, res_plsr_quad$beta, res_plsr_quad$gamma, res_plsr_quad$alpha)
y_clasif <- ifelse(y_pred_final > 0.5, 1, 0)
accuracy <- mean(y_valid == y_clasif)
cat("Accuracy final:", round(accuracy, 3), "\n")
...
```{r}
# Curva de convergencia
plot(res_plsr_2$cost_history, type = "l", lwd = 2, col = "blue",
     xlab = "Iteraciones", ylab = "Negative Log-Likelihood",
     main = "Convergencia del descenso por gradiente")

# Comparar coeficientes recuperados vs verdaderos
plot(fd(coef = res_plsr_2$beta,basisobj =  estimationBasis), col = "red", lwd = 2)
legend("topright", legend = c("PLSR Quad"), col = c("red"), lty = 1, lwd = 2)

evalbasis <- eval.basis(t,estimationBasis)
#evalbasis <- eval.fd(t,basis)
# Visualizar con persp (superficie 3D base)
gamma_est_plsr_quad <- evalbasis %*% res_plsr_2$gamma %*% t(evalbasis )
gamma_est_plsr_noquad <- evalbasis %*% res_plsr_noquad$gamma %*% t(evalbasis )
...
# Medir accuracy de predicción
y_pred_final <- predict_log_func(fd_centered = fd_valid_centered, psi, res_plsr_2$beta, res_plsr_2$gamma, res_plsr_2$alpha)
y_clasif <- ifelse(y_pred_final > 0.5, 1, 0)
accuracy <- mean(y_valid == y_clasif)
cat("Accuracy final:", round(accuracy, 3), "\n")
