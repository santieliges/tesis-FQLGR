...
# La simu "original"
## El problema que tuve es que acá estoy tomando de manera aleatoria a las coordenadas \beta y \gamma, entonces cuando cambie el tamaño o la cantidad de knots de la base, cambian las funciones parametricas en casa escenario
```{r}

#-------------------------------------------------------------
# Simulación de Datos Funcionales con Base B-Spline
#-------------------------------------------------------------
library(fda)
library(MASS)
...
dt <- t[2] - t[1]
nbasis <- 15
basis <- create.bspline.basis(rangeval = c(-1.5, 1.5), nbasis = nbasis)

mean_coef <- rnorm(nbasis, sd = 1:nbasis)
cov_coef <- diag(rev(1:nbasis))
coef <- mvrnorm(n, mu = mean_coef, Sigma = cov_coef)
fd_obj <- fd(coef = t(coef), basisobj = basis)

# Visualización de las curvas simuladas
plot(fd_obj, main = "Datos Funcionales Simulados", col = rgb(0.2, 0.4, 0.6, 0.3))
mean_fd <- mean.fd(fd_obj)
lines(mean_fd, col = "red", lwd = 3)

#-------------------------------------------------------------
# Preprocesamiento: Centrado de Curvas
#-------------------------------------------------------------
fd_centered <- center.fd(fd_obj)
par(mfrow = c(1, 2))
plot(fd_obj, main = "Datos Originales", col = "gray")
lines(mean(fd_obj), col = "red", lwd = 2)

plot(fd_centered, main = "Datos Centrados", col = "gray")
abline(h = 0, col = "red", lwd = 2)

#-------------------------------------------------------------
# Generación de Coeficientes Verdaderos (β y γ)
#-------------------------------------------------------------
simular_superficie_suave <- function(s_grid, t_grid) {
  # Malla de puntos
  grid_s <- rep(s_grid, each = length(t_grid))
  grid_t <- rep(t_grid, times = length(s_grid))
  
  # Gaussiana 2D
  gauss2d <- function(s, t, mu_s, mu_t, sigma = 0.15) {
    exp(-((s - mu_s)^2 + (t - mu_t)^2) / (2 * sigma^2))
  }
  
  # Superficie combinada
  gamma_vals <- (
    gauss2d(grid_s, grid_t, 0.3, 0.3) +
    1.2 * gauss2d(grid_s, grid_t, 0.7, 0.5) -
    0.8 * gauss2d(grid_s, grid_t, 0.5, 0.8)
  )
  
  matriz_gamma <- matrix(gamma_vals, nrow = length(s_grid), ncol = length(t_grid))
  return(matriz_gamma)
}

# β(t)
coef_beta <- rep(0, nbasis)
coef_beta[2] <- 1
coef_beta[5] <- -3
coef_beta[8] <- 0.67

evalbasis <- eval.basis(t, basis)
matrix_beta <- evalbasis %*% coef_beta

# γ(s,t)
set.seed(123)
coef_gamma <- matrix(0, nrow = nbasis, ncol = nbasis)
for (i in 2:(nbasis - 2)) {
  for (j in (i + 1):(nbasis - 1)) {
    randnum <- runif(1, -3, 3)
    coef_gamma[i, j] <- randnum
    coef_gamma[j, i] <- randnum
  }
}
diag(coef_gamma) <- 0
coef_gamma[1, ] <- 0
...
matriz_gamma <- evalbasis %*% coef_gamma %*% t(evalbasis)

#-------------------------------------------------------------
# Visualización de β(t) y γ(s,t)
#-------------------------------------------------------------
plot(t, matrix_beta, type = "l", main = "β(t)", xlab = "t", ylab = "β(t)", lwd = 2, col = "blue")

persp(x = t, y = t, z = matriz_gamma,
      theta = 40, phi = 30, expand = 0.8,
      col = "lightblue",
      xlab = "s", ylab = "t", zlab = "γ(s,t)",
      ticktype = "detailed",
      cex.main = 1.1, cex.lab = 0.9, cex.axis = 0.7)

#-------------------------------------------------------------
#  Producto Interno de la Base
#-------------------------------------------------------------
psi <- inprod(basis, basis)

```

# Simulación más densa
## Los plots que estan en la carpeta de plots actual salen de esta simu!
## Aca pongo más puntos en la grilla (pasa de 100 a 2000) y una base con más elementos. además defino a las funciones parametricas y luego estimo sus coordenadas en la base, de esta forma es el mismo caso aun que cambie el tamaño de la base)
```{r}
# =====================================================================
# Simulación de Datos Funcionales Correlacionados base densa
# =====================================================================
...
# base con orden máximo (orden = nbasis)
...
Sigma <- Sigma * diag(seq(nbasis, 1)) * 3
...
fd_train_center = mean.fd(fd_train)
fd_valid <- fd(fd_obj[["coefs"]][, idx_valid], basis)
fd_valid_centered <- fd_valid
fd_valid_centered$coefs <- fd_valid$coefs - as.vector(fd_train_center$coef)
...
mseb_quad_fpca <- MSEB(alpha_true, modelo_fpca$alpha, coef_beta, beta_est_fpca, coef_gamma, gamma_est_fpca)
...
```{r}
param_space <- list(
    step_gradient = c(0.01,  0.03, 0.05),
    lambda_lin    = c(1e-3, 1e-2, 1e-1, 1, 10),
    lambda_quad   = c(1e-3, 1e-2, 1e-1, 1, 10),
    var_treshold    = c(0.7, 0.8, 0.9)
)
library(purrr)
library(dplyr)

randomized_grid_search <- function(
    fd_train, y_train, basis, matriz_beta, matriz_gamma,
    t, folds = 5, n_iter = 20   # n_iter = cantidad de samples aleatorios
) {
    n <- length(y_train)
    fold_id <- sample(rep(1:folds, length.out = n))

    # Random samples of hyperparameters
    samples <- tibble(
        step_gradient = sample(param_space$step_gradient, n_iter, replace = TRUE),
        lambda_lin    = sample(param_space$lambda_lin,    n_iter, replace = TRUE),
        lambda_quad   = sample(param_space$lambda_quad,   n_iter, replace = TRUE),
        var_treshold    = sample(param_space$var_treshold,    n_iter, replace = TRUE)
    )

    results <- samples %>% mutate(
        score = map_dbl(1:n_iter, function(i) {

            sg  <- samples$step_gradient[i]
            ll  <- samples$lambda_lin[i]
            lq  <- samples$lambda_quad[i]
            vt  <- samples$var_treshold[i]
            ...
            for(k in 1:folds) {
                ...
                fd_tr <- fd_train[idx_train]
                y_tr  <- y_train[idx_train]

                fd_va <- fd_train[idx_valid]
                y_va  <- y_train[idx_valid]

                # init parameters
                beta_init  <- rnorm(nbasis, sd = 0.1)
                gamma_init <- matrix(rnorm(nbasis^2, sd = 0.1), nbasis)
                alpha_init <- 0

                res <- pca_coef_Upsilon1(
                    fd_centered = fd_tr,
                    y = y_tr,
                    beta  = beta_init,
                    gamma = gamma_init,
                ...
                # ---- compute IMSEB_gamma on validation ----
                evalbasis <- eval.basis(t, basis)
                gamma_est_matriz <- evalbasis %*% res$gamma %*% t(evalbasis)

                imseb_gamma <- IMBSEB(matriz_gamma, gamma_est_matriz, t, t)

                cv_scores <- c(cv_scores, imseb_gamma)
            }
    ...
    best_idx <- which.min(results$score)
    best_params <- results[best_idx, ]

    list(results = results, best = best_params)
}
search <- randomized_grid_search(
    fd_train, y_train, basis,
    matriz_beta, matriz_gamma, t,
    folds = 5,
    n_iter = 30   # 30 random samples
)
...
mseb_quad_pca <- MSEB(alpha_true, res_pca_quad$alpha, coef_beta, res_pca_quad$beta, coef_gamma, res_pca_quad$gamma)
...
mseb_quad_ups2 <- MSEB(alpha_true, res_plsr$alpha, coef_beta, res_plsr$beta, coef_gamma, res_plsr$gamma)

res_plsr_noquad<- PLSR_Upsilon_2(
    fd_centered=fd_train,
    y= y_train,
    beta = beta_init,
    gamma = gamma_init,
...
## PLSR con comps cuadraticas Upsilon 3
```{r}
beta_init <- rnorm(nbasis, sd = 0.1)
gamma_init <- gamma_init <- matrix(rnorm(nbasis^2, sd = 0.1), nrow = nbasis)
alpha_init <- 0
...
mseb_quad_ups3 <- MSEB(alpha_true, res_plsr_2$alpha, coef_beta, res_plsr_2$beta, coef_gamma, res_plsr_2$gamma)
...
# ---------------------------------------------
# 1) FUNCIÓN PARA EVALUAR UNA CONFIGURACIÓN
# ---------------------------------------------
eval_model <- function(lambda_lin, lambda_quad, 
                       nt_lin, nt_quad,
                       nivel_sign_quad) {
  ...
  res <- FPLSR_Upsilon3(
    ...
    nt_quad = nt_quad,
    nivel_significancia_lin = 1,
    nivel_significancia_quad = nivel_sign_quad,
    scale = TRUE
  )
  
  # IMSEB de beta (por si querés guardar)
  beta_fd  <- fd(coef = res$beta, basisobj = basis)
  imseb_b  <- IMSEB_integral(eval.fd(beta_fd, t), matriz_beta, t)
  
  # IMSEB de gamma (criterio de selección)
  evalbasis <- eval.basis(t, basis)
  gamma_m   <- evalbasis %*% res$gamma %*% t(evalbasis)
  imseb_g   <- IMBSEB(matriz_gamma, gamma_m, t, t)
  ...
  list(
    ...
    hp = list(
      ...
      nt_lin = nt_lin,
      nt_quad = nt_quad,
      nivel_sign_quad = nivel_sign_quad
    )
  )
}

# ----------------------------------------------------------
# 2) RANDOM SEARCH SOBRE EL ESPACIO DE HIPERPARÁMETROS
# ----------------------------------------------------------

set.seed(123)

N_TRIES <- 30  # número de combinaciones

hp_space <- tibble(
  lambda_lin  = runif(N_TRIES, 1e-4, 1e-2),
  lambda_quad = runif(N_TRIES, 1e-4, 1e-2),
  nt_lin      = sample(10:40, N_TRIES, replace = TRUE),
  nt_quad     = sample(40:120, N_TRIES, replace = TRUE),
  nivel_sign_quad = runif(N_TRIES, 0.05, 0.25)
)

# ----------------------------------------------------------
# 3) EVALUAR TODAS LAS CONFIGURACIONES
# ----------------------------------------------------------

results <- pmap(
  hp_space,
  ~ eval_model(..1, ..2, ..3, ..4, ..5)
)

df_results <- tibble(
  imseb_beta  = map_dbl(results, "imseb_beta"),
  imseb_gamma = map_dbl(results, "imseb_gamma"),
  lambda_lin  = map_dbl(results, ~ .$hp$lambda_lin),
  lambda_quad = map_dbl(results, ~ .$hp$lambda_quad),
  nt_lin      = map_int(results, ~ .$hp$nt_lin),
  nt_quad     = map_int(results, ~ .$hp$nt_quad),
  nivel_sign_quad = map_dbl(results, ~ .$hp$nivel_sign_quad)
)

# ----------------------------------------------------------
# 4) SELECCIÓN DE LOS MEJORES 6 POR IMSEB GAMMA
# ----------------------------------------------------------

top6 <- df_results %>% arrange(imseb_gamma) %>% head(6)

print("Top 6 hiperparámetros minimizando IMSEB_gamma:")
print(top6)
...
res_noPCA <- Upsilon_naive(
    ...
    gamma = gamma_init,  alpha  = alpha_init, 
                                       step_gradient = 5, iterations = 18000,
                                       basis = basis, tol = 1e-10,
                                        LdPenalization = harmacclLdf,
...
mseb_quad_naive <- MSEB(alpha_true, res_noPCA$alpha, coef_beta, res_noPCA$beta, coef_gamma, res_noPCA$gamma)
...
## Grafico las AUCROCS, AUCPR, y las estimaciones de beta y gamma para cada caso y se guardan en la carpeta PLOTS
```{r}
...
y_prob_valid_base_fpca_noquad    <- predecir_probabilidades_validacion_en_base_fpca(modelo_fpca_lin, fd_valid, modelo_fpca_lin$fpca_model$harmonics)
...
lambdas_quad <- c(0, 1e-6, 1e-4, 1e-2, 1e-1)
...
#------------------------------------------------------------
mse_vals <- imseb_vals <- imseg_vals <- auc_roc_vals <- auc_pr_vals <- numeric(length(lambdas_quad))
roc_data <- list()

#------------------------------------------------------------
# 4. Bucle sobre las penalizaciones lineales
#------------------------------------------------------------
for (i in seq_along(lambdas_quad)) {
  lambda_q <- lambdas_quad[i]
  cat("\n=== Entrenando modelo con λ_quad =", lambda_q, "===\n")
  
  # Definir penalización
  roughtnessPenalty <- modifyList(base_params, list(lambda_quad = lambda_q))
  ...
  modelo_pen <- fpca_Upsilon0(fd_centered=fd_train,
    ...
    LdPenalization = harmacclLdf,
    lambda_lin = 0	,
    lambda_quad = lambda_q)
  
  
  # Exportar β y γ estimados
  exportar_graficas_de_modelodpg(modelo_pen, paste0(varianza_fija, "_lambda_quad_", lambda_q), "pen")
  ...
  roc_data[[i]] <- data.frame(
    ...
    lambda = paste0("λ=", format(lambda_q, scientific = TRUE))
  )
}
...
ggplot(roc_df, aes(x = fpr, y = tpr, color = lambda)) +
  ...
  labs(
    title = bquote("Curvas ROC para distintos valores de" ~ lambda[quad] ~ 
                   "(varianza explicada =" ~ .(varianza_fija) ~ ")"),
    x = "1 - Especificidad (FPR)",
    y = "Sensibilidad (TPR)",
    color = expression(lambda[quad])
  )

ggsave(
  filename = file.path("plots/variandoPenalizacion", paste0("curvas_ROC_lambda_quad_var_", varianza_fija, ".png")),
...
#------------------------------------------------------------
metricas_df_penalizacion_quad <- data.frame(
  lambda_lin = lambdas_quad,
  MSE = mse_vals,
...
print(metricas_df_penalizacion_quad)
