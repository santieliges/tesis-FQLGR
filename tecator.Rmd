...
```{r}
load("modelos_tecator.RData")
load("resultados_cond_tecator.RData")
```
...
fd_valid_centered$coefs <- fd_valid_centered$coefs - rowMeans(fd_train$coefs)
...
# Evalúo las curvas centradas
t_grid <- t  # podés usar algo más fino p.ej. seq(min(t), max(t), length=300)

eval_train <- eval.fd(t_grid, fd_train)  # matriz: length(t_grid) x n_train

# Convertir a data frame largo
df_train_plot <- as.data.frame(eval_train)
df_train_plot$argvals <- t_grid

df_train_plot <- df_train_plot %>%
  pivot_longer(cols = -argvals, 
               names_to = "curve",
               values_to = "value") %>%
  mutate(curve = as.numeric(gsub("V", "", curve)),
         class = y_train[curve])

ggplot(df_train_plot, aes(x = argvals, y = value, group = curve, color = factor(class))) +
  geom_line(alpha = 0.7) +
  ...
  theme_minimal() +
  labs(
    title = "Curvas funcionales centradas (Train)",
    x = "Longitud de onda",
    y = "Absorción (centrada)"
  ) +
  theme(
...
print(sum(y_valid)/length(y_valid))
print(sum(y_train)/length(y_train))
```
...
library(dplyr)
library(tidyr)
library(patchwork)  # opcional para combinar plots

# --- Separar los FD por clase ---
fd_train_class0 <- fd_train[y_train == 0]
fd_train_class1 <- fd_train[y_train == 1]

# --- FPCA por clase ---
pca0 <- pca.fd(fd_train_class0, nharm = 3)
pca1 <- pca.fd(fd_train_class1, nharm = 3)

# --- Evaluar los autovectores (harmonics) ---
harm0 <- eval.fd(t_grid, pca0$harmonics)  # dim: length(t) x nharm
harm1 <- eval.fd(t_grid, pca1$harmonics)

# --- Pasar a data frame largo para ggplot ---
df0 <- as.data.frame(harm0)
df0$argvals <- t_grid
df0$class <- "Clase 0"
...
# --- Plot ---
p_fpca <- ggplot(df_pc, aes(x = argvals, y = value, color = PC)) +
  geom_line(linewidth = 1.2) +
  facet_wrap(~ class, ncol = 1, scales = "free_y") +
  theme_minimal() +
  labs(
    title = "Componentes principales funcionales por clase",
    x = "Longitud de onda",
    y = "Autovector funcional (harmonic)"
  ) +
...
ggplot(data.frame(Fat = fat), aes(x = Fat)) +
  geom_density(fill = "lightblue", alpha = 0.5) +
  geom_vline(xintercept = threshold, linetype = "dashed", size = 1) +
  labs(title = "Densidad del % de grasa",
       subtitle = paste("Mediana =", round(threshold,2)),
       x = "% de grasa",
       y = "Densidad") +
  theme_minimal()
...
res_pca_quad <- pca_coef_Upsilon1(fd_centered = fd_train,
                                  y = y_train, 
                                  beta = beta_init, 
                                  gamma = gamma_init,
                                  alpha = alpha_init,
...
res_pca_noquad <- pca_coef_Upsilon1(fd_centered = fd_train,
                                    y = y_train, 
                                    beta = beta_init,
                                    gamma = gamma_init,
                                    alpha = alpha_init,
...
res_plsr_quad <- PLSR_Upsilon_2(fd_centered = fd_train,
                                y = y_train,
                                beta = beta_init,
                                gamma = gamma_init,
                                alpha = alpha_init,
...
res_plsr_noquad <- PLSR_Upsilon_2(fd_centered = fd_train,
                                  y = y_train, 
                                  beta = beta_init,
                                  gamma = gamma_init,
                                  alpha = alpha_init,
...
res_noPCA <- Upsilon_naive(fd_centered = fd_train, y = y_train, beta = beta_init, gamma = gamma_init, alpha = alpha_init, step_gradient = 0.001, iterations = 3000, basis = estimationBasis,  LdPenalization = harmacclLfd, lambda_lin = 1, lambda_quad = 0)
```
...
y_prob_valid_pca    <- predecir_probabilidades_validacion(res_pca_quad, fd_valid, estimationBasis, modo = "PCA")
y_prob_valid_pca_noquad    <- predecir_probabilidades_validacion(res_pca_noquad, fd_valid, estimationBasis, modo = "PCA")
y_prob_valid_NoPCA  <- predecir_probabilidades_validacion(res_noPCA, fd_valid, estimationBasis, modo = "NoPCA")
y_prob_valid_plsr   <- predecir_probabilidades_validacion(res_plsr_quad, fd_valid, estimationBasis, modo = "PLSR")
y_prob_valid_plsr_2   <- predecir_probabilidades_validacion(res_plsr_2, fd_valid, estimationBasis, modo = "PLSR")
y_prob_valid_plsr_noquad   <- predecir_probabilidades_validacion(res_plsr_noquad, fd_valid, estimationBasis, modo = "PLSR")
...
# --- Mejorar ejes ---
par(
  cex.axis = 1.3,      # tamaño números eje
  col.axis = "black",  # color
  font.axis = 2,       # <<< NÚMEROS DE LOS EJES EN NEGRITA
  cex.lab = 1.4,       # tamaño etiquetas eje
  col.lab = "black",
  font.lab = 2,        # <<< ETIQUETAS "x" y "y" EN NEGRITA
  lwd = 1.5            # ejes un poco más gruesos (opcional)
)
...
legend("bottomright",
       ...
       col = c("darkred","purple", "violet", "darkgreen","lightgreen", 
               "darkblue","skyblue","#007999"),
       lwd = 2,
...
y_prob_valid_plsr_noquad <- predecir_probabilidades_validacion(res_plsr_noquad, fd_valid, estimationBasis, modo = "PLSR")
y_prob_valid_base_fpca <- predecir_probabilidades_validacion_en_base_fpca(modelo_fpca, fd_valid, modelo_fpca$fpca_model$harmonics)
y_prob_valid_base_fpca_noquad <- predecir_probabilidades_validacion_en_base_fpca(modelo_fpca_lin, fd_valid, modelo_fpca$fpca_model$harmonics)
...
resultados_y_distancias_tecator = evaluar_modelos_distintas_bases(
...
resultados_tecator <- resultados_y_distancias_tecator$resultados
distancias_tecator <- resultados_y_distancias_tecator$distancias
...
resultados_tecator <- resultados_tecator %>%
  mutate(
    modelo = case_when(
...
distancias_tecator <- distancias_tecator %>%
  mutate(
    modelo1 = case_when(
...
save(resultados_tecator, distancias_tecator, file = "resultados_cond_tecator.RData")
...
carpeta <- "plots_tecator/comparacion modelos numero cond tecator"
...
# ---- PLOT 2 ----
p2 <- ggplot(resultados_tecator, 
             aes(x = cond_psi, y = AUC,
                 color = modelo, linetype = modelo)) +
  geom_line(size = 1.2, alpha = 0.6,
            position = position_dodge(width = 0.15)) +
  geom_point(size = 2.4, alpha = 0.8,
...
# ---- PLOT 3 ----
p3 <- ggplot(resultados_tecator,
             aes(x = cond_var_fd, y = AUC,
                 color = modelo, linetype = modelo)) +
  geom_line(size = 1.2, alpha = 0.6,
            position = position_dodge(width = 0.15)) +
  geom_point(size = 2.4, alpha = 0.8,
...
# ---- PLOT 4 ----
p4 <- ggplot(resultados_tecator,
             aes(x = nbasis, y = cond_psi,
                 color = modelo, linetype = modelo)) +
  geom_line(size = 1.2, alpha = 0.6,
            position = position_dodge(width = 0.15)) +
  geom_point(size = 2.4, alpha = 0.8,
...
# ---- PLOT 5 ----
p5 <- ggplot(resultados_tecator,
             aes(x = nbasis, y = cond_var_fd,
                 color = modelo, linetype = modelo)) +
  geom_line(size = 1.2, alpha = 0.6,
            position = position_dodge(width = 0.15)) +
  geom_point(size = 2.4, alpha = 0.8,
...
# ---- PLOT 6 ----
####### plot distancias #################
dist_unicas <- distancias_tecator %>%
...
convert_to_plotmath <- function(expr_string) {
  out <- try(parse(text = expr_string), silent = TRUE)
  if(inherits(out, "try-error")) return(expr_string)
  expr_string
}

dist_largo2 <- dist_largo %>%
  mutate(
    expr1 = sapply(modelo1, convert_to_plotmath),
    expr2 = sapply(modelo2, convert_to_plotmath),
    par_expr = paste(expr1, "-", expr2)
  )

tipos <- unique(dist_largo2$tipo)
label_map <- c(
  dist_alpha = "Delta * alpha",
  dist_beta  = "Delta * beta",
  dist_gamma = "Delta * gamma",
  dist_total = "Delta * total"
)

for(t in tipos){

  title_expr <- substitute(
    paste("Evolución de ", LBL, " según nbasis"),
    list(LBL = parse(text = label_map[[t]])[[1]])
  )

  p <- dist_largo2 %>%
    filter(tipo == t) %>%
    ggplot(aes(x = nbasis, y = distancia,
               color = par_expr, linetype = par_expr)) +
    geom_line(size = 1.2) +
    geom_point(size = 2) +
    theme_minimal() +
    labs(
...
ggplot(resultados_tecator, aes(x = nbasis, y = AUC, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
      scale_color_discrete(labels = function(x) parse(text = x)) +
...
ggplot(resultados_tecator, aes(x = cond_psi, y = AUC, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
      scale_color_discrete(labels = function(x) parse(text = x)) +
...
ggplot(resultados_tecator, aes(x = cond_var_fd, y = AUC, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
      scale_color_discrete(labels = function(x) parse(text = x)) +
...
ggplot(resultados_tecator, aes(x = nbasis, y = cond_var_fd, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
      scale_color_discrete(labels = function(x) parse(text = x)) +
