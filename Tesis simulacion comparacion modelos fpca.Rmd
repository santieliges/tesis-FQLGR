...
```{r}
    ...
    escenario_ligero  <- simular_datos_funcionales(seq(-1, 1, length.out = 201), 3000, nbasis = 100, seed = 123, escenario = "ligero")
       ...
       inprod(escenario_ligero_chol$fd_train$basis,escenario_ligero_chol$fd_train$basis)

    escenario_denso_fourier   <- simular_datos_funcionales(seq(-1, 1, length.out = 201), 3000, nbasis = 11, seed = 123, escenario = "denso",tipo_basis = "fourier")
  ...
  for (nbasis in nbasis_grid) {
    ...
    for (escenario_nombre in c("denso", "ligero")) {
      ...
      if (chol) {
        ...
        modelo_fpca_equiv <- fpca_upsilon0_equiv_normalizado(
          fd_centered = fd_train,
          y = y_train,
          beta = beta_init,
          gamma = gamma_init,
        ...
        # =======================
        res_pca_quad <- pca_coef_Upsilon1_normalizado(
          fd_centered = fd_train,
          y = y_train,
          beta = beta_init,
          gamma = gamma_init,
    ...
    mse_fpca <- MSEB(escenario$alpha, modelo_fpca$alpha, escenario$coef_beta, beta_est_fpca, escenario$coef_gamma, gamma_est_fpca)
      ...
      mse_fpca_equiv = MSEB(alpha_true = escenario$alpha, alpha_hat = modelo_fpca_equiv$alpha, beta_true = escenario$coef_beta, beta_hat = modelo_fpca_equiv$beta, gamma_true = escenario$coef_gamma, gamma_hat = modelo_fpca_equiv$gamma)
      ...
      mse_pca = MSEB(alpha_true = escenario$alpha, alpha_hat = res_pca_quad$alpha, beta_true = escenario$coef_beta, beta_hat = res_pca_quad$beta, gamma_true = escenario$coef_gamma, gamma_hat = res_pca_quad$gamma)
      # =======================
      # AUC ROC
      # =======================
      ...
      for (i in 1:n_mod) {
        for (j in 1:n_mod) {
          ...
          distancias_df <- distancias_df %>% add_row(
            ...
            dist_beta  = dist_euclidea(m1$beta_reduced, m2$beta_reduced),
            dist_gamma = dist_euclidea(as.vector(m1$gamma_reduced), as.vector(m2$gamma_reduced))
          )
...
```{r}
distancias_modelos_simulados_K_fijo <- function(n = 3000,
                                      nbasis_grid = c(30, 50, 70),
                                      t,
                                      seed = 123,
                                      Lcoef = 2,
...
```{r}
library(dplyr)
resultados_y_distancias_bspline = evaluar_modelos_simulados(3000, c(10,15,30,40,50,60,70,80,90,100), t = seq(-1, 1, length.out = 201), seed = 123, Lcoef = c(3)
)
resultados_bspline <- resultados_y_distancias_bspline$resultados
distancias_bspline <- distancias_modelos_simulados_K_fijo(3000, c(10,15,30,40,50,60,70,80,90,100), t = seq(-1, 1, length.out = 201), seed = 123, Lcoef = c(0))


resultados_y_distancias_bspline_chol = evaluar_modelos_simulados(1000, c(10,15,30,40,50,60,70,80,90,100), t = seq(-1, 1, length.out = 201), seed = 123, Lcoef = c(0), chol = TRUE
)

resultados_bspline_chol <- resultados_y_distancias_bspline_chol$resultados
distancias_bspline_chol <- distancias_modelos_simulados_K_fijo(1000, c(10,15,30,40,50,60,70,80,90,100), t = seq(-1, 1, length.out = 201), seed = 123, Lcoef = c(0), chol = TRUE)
...
distancias_bspline <- distancias_bspline %>%
  mutate(
    modelo1 = case_when(
      modelo1 == "fpca_Upsilon0" ~ "Upsilon^{(0)}",
...
distancias_bspline <- distancias_bspline %>%
  mutate(
    modelo2 = case_when(
      modelo2 == "fpca_Upsilon0" ~ "Upsilon^{(0)}",
      modelo2 == "equiv" ~ "Upsilon[Equivalente]^{(0)}",
      modelo2 == "pca_quad" ~ "Upsilon^{(1)}",
      TRUE ~ modelo2
    )
  )
save(resultados_bspline, distancias_bspline, file = "resultados_condgram_simulacion_bspline.RData")
...
distancias_bspline_chol <- distancias_bspline_chol  %>%
  mutate(
    modelo1 = case_when(
...
save(resultados_bspline_chol, distancias_bspline_chol, file = "resultados_condgram_simulacion_bspline_chol.RData")
```

```{r}
load("resultados_condgram_simulacion_bspline.RData")
load("resultados_condgram_simulacion_bspline_chol.RData")
```
...
p4 <- ggplot(resultados_bspline, aes(x = nbasis, y = cond_psi, color = modelo ,linetype = escenario)) +
  ...
  labs(title = "Evolución de cond_psi según nbasis",
       x = "nbasis", y = "kappa(Psi)") + tema_negrita
...
# ------------ PLOT 7 DISTANCIAS ENTRE PARAMETROS ESTIMADOS -------------
####### plot distancias #################
dist_unicas <- distancias_bspline %>%
...
for(t in tipos){
  ...
  p <- dist_largo2 %>%
    ...
    ylab(parse(text = label_map[[t]])) + tema_negrita
...
p6 <- ggplot(resultados_bspline_chol, aes(x = nbasis, y = mse, color = modelo ,linetype = escenario)) +
  ...
  coord_cartesian(ylim = c(0, 3000)) + tema_negrita   # <-- por ejemplo
...
####### plot distancias #################
dist_unicas <- distancias_bspline_chol %>%
  mutate(par = paste0(pmin(modelo1, modelo2), " - ", pmax(modelo1, modelo2))) %>%
  filter(modelo1 != modelo2) %>%
  distinct(nbasis, par, .keep_all = TRUE)
...
resultados_fourier = evaluar_modelos_simulados(3000, c(11,15,31,41,51,61,71,81,91), t = seq(-1, 1, length.out = 301), seed = 123, Lcoef = c(0), tipo_basis = "fourier"
)
```
...
resultados_fourier_prueba = evaluar_modelos_simulados(3000, c(11,15,31), t = seq(-1, 1, length.out = 301), seed = 123, Lcoef = c(0), tipo_basis = "fourier"
)

resultados_fourier_prueba$distancias
```
...
```{r}
load("resultados_condgram_simulacion_fourier.RData")
```
...
# ---- PLOT 4 ----
p4 <- ggplot(resultados_fourier, aes(x = nbasis, y = AUC, color = modelo ,linetype = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  theme_minimal() +
...
ggsave(filename = file.path(carpeta, "AUC_vs_nbasis.png"),
       plot = p4, width = 7, height = 5, dpi = 300)
# ---- PLOT 6 ----
p6 <- ggplot(resultados_fourier, aes(x = nbasis, y = mse, color = modelo ,linetype = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  theme_minimal() +
...
```{r}
A <- t(escenario_denso_fourier$fd_train$coefs)
Psi <- inprod(escenario_denso_fourier$fd_train$basis,escenario_denso_fourier$fd_train$basis )
psi_half <- Psi_half(Psi)
psi_half_inv <- Psi_inv_half(Psi)

pca_coefs <- prcomp(A%*%psi_half, rank. = 5)
coords_fpca_equiv <- psi_half_inv%*%pca_coefs$rotation

coords_fpca <- pca.fd(escenario_denso_fourier$fd_train,nharm = 5)

Q1 <- coords_fpca$harmonics$coefs   # p x K
Q2 <- coords_fpca_equiv             # p x K
