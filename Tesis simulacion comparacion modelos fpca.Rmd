...
ggplot(resultados_bspline, aes(x = cond_psi, y = AUC, color = modelo, linetype = escenario)) +
  geom_line(size = 1.2) +
  geom_point() +
  theme_minimal() +
...
ggplot(resultados_bspline, aes(x = cond_psi, y = IMSE_beta, color = modelo, linetype = escenario)) +
  geom_line(size = 1.2) +
  theme_minimal() +
  labs(title = "Evolución del IMSE(β)", x = "Numero de condición de Ψ ", y = "IMSE β")

resultados_bspline$IMSE_beta

ggplot(resultados_bspline, aes(x = cond_psi, y = IMSE_gamma, color = modelo, linetype = escenario)) +
  geom_line(size = 1.2) +
  theme_minimal() +
  labs(title = "Evolución del IMSE(gamma)", x = "Numero de condición de Ψ ", y = "IMSE gamma")

resultados_bspline$IMSE_gamma

ggplot(resultados_bspline, aes(x = cond_psi, y = mse, color = modelo, linetype = escenario)) +
  geom_line(size = 1.2) +
  theme_minimal() +
  labs(title = "Evolución del mse", x = "Numero de condición de Ψ ", y = "MSE")

resultados_bspline$mse

ggplot(resultados_bspline, aes(x = nbasis, y = cond_psi, color = modelo, linetype = escenario)) +
  geom_line(size = 1.2) +
  theme_minimal() +
...
# ---- PLOT 1 ----
p1 <- ggplot(resultados_fourier, aes(x = nbasis, y = AUC, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
...
# ---- PLOT 2 ----
p2 <- ggplot(resultados_fourier, aes(x = cond_psi, y = AUC, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  theme_minimal() +
  labs(title = "Evolución de AUC según kappa(Psi)",
       x = "kappa(Psi)", y = "AUC ROC")

ggsave(filename = file.path(carpeta, "AUC_vs_cond_psi.png"),
       plot = p2, width = 7, height = 5, dpi = 300)

# ---- PLOT 4 ----
p4 <- ggplot(resultados_fourier, aes(x = nbasis, y = cond_psi, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  theme_minimal() +
...
# ---- PLOT 5 ----
p5 <- ggplot(resultados_fourier, aes(x = cond_psi, y = mse, color = modelo)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  theme_minimal() +
  labs(title = "Evolución de kappa(Psi) según mse",
       x = "kappa(Psi)", y = "mse")

ggsave(filename = file.path(carpeta, "msevskappa(psi).png"),
...
resultados_fourier$base <- "fourier"

# Combinar en un único dataframe
resultados_total <- rbind(resultados_bspline, resultados_fourier)

p_comb <- ggplot(resultados_total,
                 aes(x = nbasis,
                     y = AUC,
                     color = modelo,
                     linetype = escenario)) +
  ...
  geom_point(size = 2) +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  scale_linetype_manual(values = c("bspline" = "solid",
                                   "fourier" = "dashed")) +
  theme_minimal() +
  labs(title = "Evolución de AUC según nbasis",
       x = "nbasis",
       y = "AUC ROC",
       linetype = "Base funcional")

ggsave(filename = file.path(carpeta, "AUC_vs_nbasis.png"),
       plot = p_comb, width = 7, height = 5, dpi = 300)
...
# ---- PLOT 2 ----
p2 <- ggplot(resultados_bspline, aes(x = cond_psi, y = AUC, color = modelo,linetype = escenario)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  theme_minimal() +
  labs(
    title = expression("Evolución de AUC según " * kappa(Psi)),
...
# ---- PLOT 5 ----
p5 <- ggplot(resultados_bspline, aes(x = cond_psi, y = mse, color = modelo ,linetype = escenario)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_color_discrete(labels = function(x) parse(text = x)) +
  theme_minimal() +
